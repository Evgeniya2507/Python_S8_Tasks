...
def input_data():
    ...
    while var != 1 and var != 2:
        print('Варианта всего два!')
        var = int(input("Введите номер варианта: "))

    if var == 1:
        with open('phonebook1.csv', 'a', encoding='utf-8') as file:
            file.write(f'{name}\n{surname}\n{phone}\n{address}\n\n')
    else:
        with open('phonebook2.csv', 'a', encoding='utf-8') as file:
...
def delete_data():
    ...
    while number_file != 1 and number_file != 2:
        print('Файла под таким номером не существует. ')
        number_file = int(input('Введите номер файла: '))
    ...
    if number_file == 1:
        ...
        if number_journal <= len(data_first):
            ...
            with open('phonebook1.csv', 'w', encoding='utf-8') as file:
                file.write(''.join(data_first))
            print('Изменения успешно сохранены!')
        else:
            print('Записи под таким номером не существует в выбранном файле')
    else:
        print("Какую именно запись по счету Вы хотите удалить?")
        number_journal = int(input('Введите номер записи: '))
        if number_journal <= len(data_second):
            ...
            with open('phonebook2.csv', 'w', encoding='utf-8') as file:
...
def print_data():
    ...
    with open('phonebook1.csv', 'r', encoding='utf-8') as file:
    ...
    with open('phonebook2.csv', 'r', encoding='utf-8') as file:
...
def show_data():
    print('Из какого файла Вам нужно вывести данные?')
    data_file = int(input('Введите номер файла: '))
    while data_file < 1 or data_file > 2:
